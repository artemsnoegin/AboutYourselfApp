...
class LocationVC: UIViewController {
    ...
    private func setUpUI() {
        ...
        descriptionLabel.text = "Где ты живёшь?"
        ...
        locationField.font = .preferredFont(forTextStyle: .title3)
        locationField.autocapitalizationType = .words
        
        warningLabel.text = "Введите название города"
        ...
        [storyLabel, descriptionLabel, locationField, warningLabel, continueButton].forEach {
            view.addSubview($0)
        ...
        NSLayoutConstraint.activate([
            ...
            descriptionLabel.bottomAnchor.constraint(equalTo: locationField.topAnchor, constant: -8),
            descriptionLabel.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 16),
            descriptionLabel.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -16),
            
            locationField.centerYAnchor.constraint(equalTo: view.centerYAnchor),
            locationField.heightAnchor.constraint(equalToConstant: 40),
            locationField.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 16),
            locationField.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -16),
            
            warningLabel.topAnchor.constraint(equalTo: locationField.bottomAnchor, constant: 8),
            warningLabel.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 22),
            
            continueButton.topAnchor.constraint(equalTo: locationField.bottomAnchor, constant: 8),
    ...
    @objc private func goToNextScreen() {
        if let location = locationField.text {
            if location != "" {
                let updatedStory = "\(story)\nЯ живу в городе \(location)"
                navigationController?.pushViewController(HobbyVC(story: updatedStory), animated: true)
            } else {
